# Beacon chain
* == 💡blockchain / introduced proof-of-stake & validators | Ethereum💡  
* | 
...
* == database of transactions /
  * duplicated & shared | ALL network's computerS
    * Reason: 🧠ensure data can NOT be altered retroactively 🧠
* == sequence of [blockS](#block) / 
  * EACH linked -- to -- its predecessor
    * -- till -- [genesis-block](#genesis-block)
    * 👀by -- referencing the -- PREVIOUS block's hash 👀
* 's integrity 
  * crypto-economically secured == cryptography + economical benefits
        
# Block
* := place | transactions or digital actions are stored
  * if the block is full -> gets linked -- to the -- PREVIOUS one
    * == creating a blockchain
  * proposed -- by -- [proof-of-stake validators](#validator) /
    * | ONCE validated, 
      * shared ACROSS the entire peer-to-peer network
      * independently verified -- by -- ALL OTHER nodes
* == bundled unit of information == 👀ordered list of transactions + consensus-related information👀
  * ordered list of transactions == CURRENT state of the network        

# Consensus
* == 👀if > 2/3 of computers | network agree about SAME set of records -> make sure everyone have SAME set of records👀
  * != [consensus rules](#consensus-rules)

## Consensus client
* _Examples:_ Prysm, Teku, Nimbus, Lighthouse, Lodestar
* run Ethereum's [proof-of-stake](#proof-of-stake-pos)
  * enable
    * network can reach agreement -- about the -- head of the Beacon Chain
* NOT 
  * participate | validating/broadcasting transactions or executing state transitions
    * done -- by -- [execution clients](#execution-client-or-eth1-clients)
  * attest to, or propose new blocks
    * done -- by -- [validator client](#validator)

## Consensus layer
* == consensus clients' network

## Consensus rules
* == 👀block validation rules👀 / 
  * full nodes follow -- to stay in consensus with -- OTHER nodes
  * != [consensus](#consensus)
  * == govern block's contents / are valid

# Execution client or Eth1 clients
* _Example:_ Besu, Erigon, Go-Ethereum (Geth), Nethermind
* in charge of
  * processing & broadcasting transactions
  * managing Ethereum's state
* run the computations / EACH transaction 
  * -- by -- using the EVM
    * Reason: 🧠ensure the rules of the protocol are followed🧠

# Genesis block
* == FIRST block | blockchain
* uses
...
* == node | [proof-of-stake](#proof-of-stake-pos) system /
  * responsible for
    ...
    * adding NEW blocks | blockchain
  * requirements
    * ⚠️stake 32 ETH⚠️
* == [consensus client](#consensus-client) / has add-on 
